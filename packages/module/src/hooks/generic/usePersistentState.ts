...
export const usePersistentState = <TValue, TPersistenceKeyPrefix extends string, TURLParamKey extends string>(
  options: UsePersistentStateOptions<TValue, TPersistenceKeyPrefix, TURLParamKey>
): [TValue, (value: TValue) => void] => {
  const { defaultValue, persistTo, persistenceKeyPrefix, isEnabled = true } = options;

  const isUrlParamsOptions = (
    o: typeof options
  ): o is PersistToUrlParamsOptions<TValue, TPersistenceKeyPrefix, TURLParamKey> => o.persistTo === 'urlParams';

  const isStorageOptions = (o: typeof options): o is PersistToStorageOptions<TValue> =>
    o.persistTo === 'localStorage' || o.persistTo === 'sessionStorage';

  const prefixKey = (key: string) => (persistenceKeyPrefix ? `${persistenceKeyPrefix}:${key}` : key);
