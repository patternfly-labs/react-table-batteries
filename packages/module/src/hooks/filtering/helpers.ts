...
export const serializeFilterUrlParams = <TFilterCategoryKey extends string>(
  ...
  objectKeys(trimmedFilterValues).forEach((filterCategoryKey) => {
    if (!trimmedFilterValues[filterCategoryKey] || trimmedFilterValues[filterCategoryKey]?.length === 0) {
 ...
 */
export const deserializeFilterUrlParams = <TFilterCategoryKey extends string>(serializedParams: {
  filters?: string | null;
